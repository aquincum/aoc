...
impl FromStr for Blueprint {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let obsidian_ore = caps
            .get(4)
            .ok_or("No obsidian_ore")?
            .as_str()
            .parse()
            .map_err(|e: ParseIntError| e.to_string())?;
        let obsidian_clay = caps
            .get(5)
            .ok_or("No obsidian_clay")?
            .as_str()
            .parse()
            .map_err(|e: ParseIntError| e.to_string())?;
        let geode_ore = caps
        ...
        rng.fold(
            ...
            |acc, i| {
                ...
                let options = acc.iter().flat_map(|s| {
                    ...
                    [Some(no_build), new_ore, new_clay, new_obsidian, new_geode]
                        .into_iter()
                        .filter_map(|x| *x)
                        .collect::<Vec<State>>()
                });
                let future_options: HashSet<State> = options.collect();
                let best_geode = future_options
                    .iter()
...
pub fn question(input: &str, question: Question) {
    ...
    let result: Quantity = match question {
        Question::First => blueprints
            .iter()
            .map(|b| b.run_blueprint(24) * b.id as u32)
            .sum(),
