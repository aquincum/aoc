...
impl FromStr for Valve {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let flow = caps.get(2).ok_or("No valve")?.as_str().parse().map_err(|e: ParseIntError| e.to_string())?;
        let tunnels = caps.get(3).ok_or("No tunnels")?.as_str();
        let tunnels = tunnels.split(", ").map(|s| s.to_string()).collect::<Vec<_>>();
        Ok(Valve{
            id: id.clone(),
            flow,
...
fn read_valve_map(input: &str) -> ValveMap {
    input.split("\n").map(|l| l.parse::<Valve>().unwrap()).map(|v| (v.get_id(), v)).collect()
}
...
pub fn question_fail1(input: &str) {
    ...
    for minute in 0..30 {
        ...
        for vid in valve_map.keys() {
            ...
            let best_tunnel_id = my_valve.tunnels.iter().max_by_key(|t| state.get(t).unwrap().sum()).unwrap();
            ...
            if minute == 0 || my_valve.flow == 0 || am_i_opened || best_tunnel.sum() > current_place.sum() + my_valve.flow {
                let msg = format!("==Minute {}==\nI am at {}. Next round we'll move to {}.", 30-minute, vid, best_tunnel_id);
                new_state.insert(vid, ValveState{
                ...
                let msg = format!("==Minute {}==\nI am at {} and I open myself", 30-minute, vid);
                new_state.insert(vid, ValveState{
                    open: [current_place.open.to_owned(), vec![vid.to_string()]].concat(),
                    releases: [vec![0] ,current_place.releases.iter().map(|r| r+my_valve.flow).collect_vec()].concat(),
            ...
            if vid == "AA" {
                println!("{:?} {:?}", new_state.get(vid).unwrap().releases, new_state.get(vid).unwrap().sum());
                let ns = new_state.get(vid).unwrap();
                let opt = ns.sum() + ((30-ns.releases.len()) as u32)*ns.releases.last().unwrap_or(&0);
                println!("OPT: {}", opt);
    ...
    println!("{}", aa_state.debug.iter().zip(&aa_state.releases.iter().rev().collect_vec()).map(|(msg, r)| format!("{}\nReleasing {} now", msg, r)).join("\n\n"));

    println!("For AA the state is open={}; sum={}", aa_state.open.join(","), aa_state.sum());
}
