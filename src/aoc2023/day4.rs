...
impl FromStr for Card {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let id = idpart
            .parse()
            .map_err(|e: ParseIntError| format!("in {}: {}", idpart, e.to_string()))?;
        let (winning, picked) = parts
            .get(1)
            .ok_or("no : long enough")?
            .split(" | ")
            .collect_tuple()
            .ok_or("no |")?;
        let winning: Result<Vec<usize>, String> = winning
            .split_ascii_whitespace()
            .map(|num| {
                num.parse()
                    .map_err(|e: ParseIntError| format!("in {}: {}", num, e.to_string()))
            })
            .collect();
        let winning = winning?;
        let picked: Result<Vec<usize>, String> = picked
            .split_ascii_whitespace()
            .map(|num| {
                num.parse()
                    .map_err(|e: ParseIntError| format!("in {}: {}", num, e.to_string()))
...
fn q2(input: &str) -> Result<u32, String> {
    let cards: Result<Vec<Card>, String> = input.lines().map(|x| x.parse()).collect();
    let cards = cards?;
    let finalstate = cards.iter().fold(
        BuyAheadState {
            future: vec![],
            cards: 0,
        },
