...
impl FromStr for Game {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if let Some((game_id_str, draws_str)) = s.split(": ").collect_tuple() {
            ...
            let draws: Result<Vec<Draw>, String> = draws_str
                .split("; ")
                .map(|draw_str| draw_str.parse())
                .collect();
            let draws = draws?;
            Ok(Game { id, draws })
        } else {
...
impl Game {
    ...
    fn fewest_cubes(&self) -> Draw {
        self.draws.iter().fold(
            ...
            |sofar, draw| Draw {
                greens: if sofar.greens > draw.greens {
                    sofar.greens
                } else {
                    draw.greens
                },
                blues: if sofar.blues > draw.blues {
                    sofar.blues
                } else {
                    draw.blues
                },
                reds: if sofar.reds > draw.reds {
                    sofar.reds
                } else {
                    draw.reds
                },
